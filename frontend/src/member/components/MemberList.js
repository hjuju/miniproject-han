...
const MemberList = () => {
  ...
  const [members, setMembers] = useState([]) //밑에꺼 보고 배열이면 []으로 설정, 파이썬에서 리스트로
  ...
  useEffect( () => {
    memberList()
    .then(res => {
      console.log(res.data) //data = 캡슐화 된 data
      setMembers(res.data)
  ...
  return (<>
    ...
    <div className={pageClasses.root}>
      <Pagination count={100} size="small" text-align="center" />
    </div>
    </>);
}
