...
def member(request, pk):
    try:
        print(f'------ {now.strftime("%Y-%m-%d %H:%M:%S")} ------')  # 디버깅 코딩 (시간)
        ...
        return JsonResponse({'result': 'MEMBER-NOT-EXISTS'}, status=201)  # 디버깅 코드(오류 난 것 인지)
    ...
    if request.method == 'GET':
        ...
        MemberVO.objects.get(username=pk).delete()  # 지우는 코드 한 줄로 가능
...
def login(request):
    try:
        data = request.data['body']  # .data == 상태 /  data == JSON
        pk = data['username']
